...
def confirm_step(step_num, description):
    """
    步骤执行确认
    ...
    print("=" * 60)
    print(f"输入 'y' 继续执行步骤{step_num}")
    print(f"输入 'n' 退出脚本")
    print("=" * 60)

    while True:
        try:
            choice = input(f"请选择 (y/n): ").strip().lower()
            if choice == 'y':
                return True
            elif choice == 'n':
                print(f"用户选择退出脚本")
...
def main():
    ...
    # 步骤1确认：反编译
    if not confirm_step(1, "反编译APK - 将APK反编译为可修改的目录"):
        return
    ...
    # 步骤2确认：修改AndroidManifest.xml
    if not confirm_step(2, "修改AndroidManifest.xml - 替换清单文件中的包名"):
        return
    ...
    # 步骤3确认：替换所有文件中的包名
    if not confirm_step(3, "替换所有文件中的包名 - 多进程并行处理 smali/xml 等文件"):
        return
    ...
    # 步骤4确认：重新打包
    if not confirm_step(4, "重新打包APK - 将修改后的目录重新打包为APK"):
        return
